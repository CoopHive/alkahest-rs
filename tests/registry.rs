...
async fn test_erc20_addresses() -> eyre::Result<()> {
    ...
    let eas_addr = context.alice_client.erc20().address(Erc20Contract::Eas);
    let barter_utils_addr = context
        .alice_client
        .erc20()
        .address(Erc20Contract::BarterUtils);
    let escrow_addr = context
        .alice_client
        .erc20()
        .address(Erc20Contract::EscrowObligation);
    let payment_addr = context
        .alice_client
        .erc20()
        .address(Erc20Contract::PaymentObligation);
...
async fn test_all_module_addresses() -> eyre::Result<()> {
    ...
    let erc721_barter = client.erc721().address(Erc721Contract::BarterUtils);
    let erc721_payment = client.erc721().address(Erc721Contract::PaymentObligation);
    assert_eq!(
        erc721_barter,
    ...
    let erc1155_escrow = client.erc1155().address(Erc1155Contract::EscrowObligation);
    let erc1155_payment = client.erc1155().address(Erc1155Contract::PaymentObligation);
    assert_eq!(
        erc1155_escrow,
    ...
    let bundle_barter = client
        .token_bundle()
        .address(TokenBundleContract::BarterUtils);
    let bundle_escrow = client
        .token_bundle()
        .address(TokenBundleContract::EscrowObligation);
    assert_eq!(
        bundle_barter,
    ...
    let string_obligation = client
        .string_obligation()
        .address(StringObligationContract::Obligation);
    assert_eq!(
        string_obligation,
...
async fn test_address_consistency_between_clients() -> eyre::Result<()> {
    ...
    let alice_attestation = context
        .alice_client
        .attestation()
        .address(AttestationContract::BarterUtils);
...
async fn test_contract_enum_ergonomics() -> eyre::Result<()> {
    ...
    let addresses = vec![
        ("ERC20 EAS", client.erc20().address(Erc20Contract::Eas)),
        ("ERC20 Barter", client.erc20().address(BarterUtils)),
        ("ERC20 Escrow", client.erc20().address(EscrowObligation)),
        ("ERC20 Payment", client.erc20().address(PaymentObligation)),
    ];
    ...
    let erc721_escrow = client.erc721().address(E721::EscrowObligation);
    println!("ERC721 Escrow: {}", erc721_escrow);

    // Direct enum usage for arbiters
    let arbiter_addrs = vec![
        client.arbiters().address(TrustedPartyArbiter),
        client.arbiters().address(TrivialArbiter),
        client.arbiters().address(TrustedOracleArbiter),
    ];
...
async fn test_direct_client_access_comparison() -> eyre::Result<()> {
    ...
    // New API
    let escrow_via_api = client.erc20().address(Erc20Contract::EscrowObligation);
...
async fn test_contract_instance_creation() -> eyre::Result<()> {
    ...
    let escrow_addr = client.erc20().address(Erc20Contract::EscrowObligation);
    ...
    let barter_addr = client.erc721().address(Erc721Contract::BarterUtils);
    let barter_contract =
